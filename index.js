...
const QUESTIONS = [
  {
    ...
    description:
      "Given two integers, add them without using addition or subtraction operators",
    testcases: [
      {
        input: "1 2",
        output: "3",
...
app.post("/login", function (req, res) {
  ...
  if (user && user.password === password) {
    ...
    return res
      .status(200)
      .sendStatus("You are logged in successfully " + token);
  } else {
    return res
      .status(401)
      .sendStatus("Email or password is incorrect for the user");
  }
});
...
app.get("/submissions", function (req, res) {
  ...
  const submissions = SUBMISSION.filter(
    (submission) => submission.questionTitle === questionTitle
  );
...
app.post("/submissions", function (req, res) {
  ...
  if (title.length % 2 == 0) accepted = true;
